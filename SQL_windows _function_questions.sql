...
-- unique value assign for every customer.
...
-- rank a order within each category based on order amount from highest to lowest.
select category, order_id, amount,
rank() over(partition by category order by amount desc) from orders;

-- find the difference bwteen each order amount and the average amount of its category.
...
-- find the higest -value order for every customer.
select * from 
...
select customer_name, order_id, amount, 
sum(amount) over(partition by customer_name ) as total_spending, 
round(  amount/sum(amount) over (partition by customer_name  ) *100,2) as percentage from orders;