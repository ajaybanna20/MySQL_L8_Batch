...
select * ,lead(amount,1) over() from orders; -- lead 
select *,lag(amount,1) over() from orders; -- lag

select *,lead(amount,1) over() ,lag(amount,1) over() from orders;

select *,lead(amount,1) over(partition by customer_id order by order_date),
lag(amount,2) over(partition by customer_id order by order_date) from orders;

select *,lag(amount,1,0) over(partition by customer_id order by order_date) as prev1,
lag(amount,2,0) over(partition by customer_id order by order_date) as prev2, 
amount + lag(amount,1,0) over(partition by customer_id order by order_date) +
lag(amount,2,0) over(partition by customer_id order by order_date)  total_prev1_pre2 from orders;

select customer_id, customer_name, amount,
sum(amount) over(rows between unbounded preceding and current row) from orders;

select customer_id, customer_name, amount,
sum(amount) over(rows between unbounded preceding and current row ) from orders; -- unbounded preceding means it start from 1st value to current row

select customer_id, customer_name, amount,
sum(amount) over(rows  between 2 preceding  and current row ) from orders; --  n preceding means where evere u want to start to current row

select order_id ,customer_id, customer_name,order_date, amount,
sum(amount) over(partition by customer_id order by order_date
rows between 2 preceding and current row) from orders; -- same pre1_pre2 question it calcualte previous 2 value 
rows between 2 preceding and current row) from orders; -- same pre1_pre2 question it calcualte previous 2 value 

select order_id ,customer_id, customer_name,order_date, amount,
sum(amount) over(partition by customer_id order by order_date
rows between 1 preceding and 1 following ) from orders; -- 1 preceding , current row and next one 