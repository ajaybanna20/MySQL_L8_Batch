...
-- Employees whose salary is greter than the maximum salary of their own department.
...
-- Employees whose salary is greter than the average salary of their own department.
select e.emp_id, e.emp_name, e.salary, d.dept_name from employee as e
join department as d on e.dept_id=d.dept_id
where salary>(select avg(salary) from employee where dept_id=e.dept_id);

-- Employees who earn more than every other employee in their department.
select e.emp_name  from employee as e
where salary>=(select max(salary) from employee where dept_id=e.dept_id);

-- Employees who are not the higest-paid employee in their department.
select e.emp_name  from employee as e
where salary!=(select max(salary) from employee where dept_id=e.dept_id);

-- department having at least one employee earing more than 70
